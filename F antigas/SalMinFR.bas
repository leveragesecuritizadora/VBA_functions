Attribute VB_Name = "SalMinFR"
Function PreencheMinFR( _
    ...
    Optional nome_fonte As String = "SalMinFR" _
) As Variant
    ...
    If wsBDconnected Is Nothing Then
        PreencheMinFR = "Erro: Tabela '" & nome_fonte & "' n�o existe"
    ...
    Else
        PreencheMinFR = "Erro: c_lula B" & celAtual.Row & " n�o cont_m uma data v�lida"
        Exit Function
    End If

    ' --- [4] Verifica se o deslocamento de ms est� dentro do intervalo ----
    If mes_offset < -12 Or mes_offset > 12 Then
        PreencheMinFR = "Erro: mes_offset fora do intervalo (-12 a 12)"
    ...
    PreencheMinFR = wsBDconnected.Cells(linhaEncontrada, colunaInfo).Value
...
ErroHandler:
    PreencheMinFR = place_holder
    ' Call LogErroUDF("ERRO em PreencheMinFR: " & Err.Description)
End Function
