...
Function PreenchePrevisto( _
    ...
    Set wsPrevistosDevidos = ThisWorkbook.Sheets(nome_fonte)
    On Error GoTo ErroHandler
    If wsPrevistosDevidos Is Nothing Then
        PreenchePrevisto = "Erro: Tabela '" & nome_fonte & "' n�o existe"
        Exit Function
    End If
    ...
    If mes_offset < -12 Or mes_offset > 12 Then
        PreenchePrevisto = "Erro: mes_offset fora do intervalo (-12 a 12)"
        Exit Function
    End If
    
    ' --- [5] Monta a string de busca ---
    ' Debug.Print "chegeu 5";
    dataBusca = Format(DateSerial(Year(dataBase), Month(dataBase) + mes_offset, 1), "dd/mm/yyyy") & " - " & unidade & " - " & tipo_devido
    
    ' --- [6] Busca o valor na planilha fonte ---
    ' Debug.Print "chegeu 6";
    linhaEncontrada = Application.Match(dataBusca, wsPrevistosDevidos.Range("B:B"), 0)
    
    If Not IsMissing(dado_historico) Then
        If Not IsEmpty(dado_historico) And dado_historico <> "" Then
            PreenchePrevisto = dado_historico
            Exit Function
        End If
    End If
    
    ' --- [8] Retorna o valor encontrado ---
    ' Debug.Print "chegeu 8"; linhaEncontrada
    PreenchePrevisto = wsPrevistosDevidos.Cells(linhaEncontrada, colunaInfo).Value
    ' Debug.Print "Busca: " & dataBusca
    ' Debug.Print "Linha encontrada: " & linhaEncontrada
    ' Debug.Print "Valor na c_lula: " & wsPrevistosDevidos.Cells(linhaEncontrada, colunaInfo).Address & " = " & wsPrevistosDevidos.Cells(linhaEncontrada, colunaInfo).Value
