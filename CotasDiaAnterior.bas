Attribute VB_Name = "CotasDiaAnterior"
Function RetornaCotasAnteriorPagamento( _
    ...
    If wsVP Is Nothing Then
        RetornaCotasAnteriorPagamento = "Erro: Tabela '" & nome_fonte & "' n�o existe"
        Exit Function
    End If
    ...
    Else
        RetornaCotasAnteriorPagamento = "Erro: c_lula B" & celAtual.Row & " n�o cont_m uma data v�lida"
        Exit Function
    End If
    ...
    If mes_offset < -12 Or mes_offset > 12 Then
        RetornaCotasAnteriorPagamento = "Erro: mes_offset fora do intervalo (-12 a 12)"
        Exit Function
    End If
    ...
    ' Debug.Print "chegeu 6 "; dataBusca
    linhaEncontrada = Application.Match(dataBusca, wsVP.Range("D:D"), 0)
    
    If Not IsMissing(dado_historico) Then
        If Not IsEmpty(dado_historico) And dado_historico <> "" Then
            RetornaCotasAnteriorPagamento = dado_historico
            Exit Function
        End If
    End If
    
    ' --- [8] Retorna o valor encontrado ---
    ' Debug.Print "chegeu 8 cotas"; linhaEncontrada
    RetornaCotasAnteriorPagamento = wsVP.Cells(linhaEncontrada, colunaInfo).Value
