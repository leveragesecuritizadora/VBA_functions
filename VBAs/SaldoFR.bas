...
Function PreencheSaldoFR( _
    ...
    Optional nome_fonte As String = "SalFR" _
) As Variant
    ...
    If wsBDconnected Is Nothing Then
        PreencheSaldoFR = "Erro: Tabela '" & nome_fonte & "' n�o existe"
    ...
    Else
        PreencheSaldoFR = "Erro: c_lula B" & celAtual.Row & " n�o cont_m uma data v�lida"
        Exit Function
    End If

    ' --- [4] Verifica se o deslocamento de ms est� dentro do intervalo ----
    If mes_offset < -12 Or mes_offset > 12 Then
        PreencheSaldoFR = "Erro: mes_offset fora do intervalo (-12 a 12)"
    ...
    If Not IsMissing(dado_historico) Then
        If Not IsEmpty(dado_historico) And dado_historico <> "" Then
            PreencheSaldoFR = dado_historico
            Exit Function
        End If
    End If
    ...
    PreencheSaldoFR = wsBDconnected.Cells(linhaEncontrada, colunaInfo).Value
...
ErroHandler:
    PreencheSaldoFR = place_holder
    ' Call LogErroUDF("ERRO em PreencheSaldoFR: " & Err.Description)
End Function
